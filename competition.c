...
enum {
  ...
  step_29_forward_to_align_line,

  step_30_rotate_90_degrees_to_line,
  step_31_forward_until_box,
  step_32_rotate_to_align_wall,
  step_32_followwall_right,
  step_33_forward,
  step_34_turn_90,
  step_35_forward_until_box_side,
  step_36_turn_90,
...
int turn(int stop_criteria, double angle, double speed, double startangle,
         int time);
int turnr(int stop_criteria, double dist, double speed, double turn_speed,
          int time);
  ...
  while (running) {
    ...
    case ms_change_step:
      ...
      if (current_step == step_1_followline_until_box) {
        ...
        angle = 0.5 * M_PI;
        current_step = step_20_rotate_90_degrees_to_align_wall;
        mission.state = ms_turn;
        ...
        mission.state = ms_followline;
      } else if (current_step == step_28_followline_white) {
        dist = 0.2;
        current_step = step_29_forward_to_align_line;
        mission.state = ms_fwd;
      } else if (current_step == step_29_forward_to_align_line) {
        angle = -0.5 * M_PI;
        current_step = step_30_rotate_90_degrees_to_line;
        mission.state = ms_turn;
      } else if (current_step == step_30_rotate_90_degrees_to_line) {
        followdirection = follow_middle;
        line_color = line_black;
        stop_criteria = stop_at_box;
        current_step = step_31_forward_until_box;
        mission.state = ms_followline;
        // Checkpoint 4
      } else if (current_step == step_31_forward_until_box) {
        angle = 0.5 * M_PI;
        current_step = step_32_rotate_to_align_wall;
        mission.state = ms_turn;
      } else if (current_step == step_32_rotate_to_align_wall) {
        followdirection = wall_left;
        current_step = step_32_followwall_right;
        mission.state = ms_followwall;
      } else if (current_step == step_32_followwall_right) {
        dist = 0.4;
        current_step = step_33_forward;
        mission.state = ms_fwd;
      } else if (current_step == step_33_forward) {
        angle = -0.5 * M_PI;
        current_step = step_34_turn_90;
        mission.state = ms_turn;
        ...
        mission.state = ms_fwd;
      } else if (current_step == step_37_forward_until_wall) {
        angle = -0.5 * M_PI;
...
void update_motcon(motiontype *p, odotype *odo) {
  ...
  if (p->stop_criteria == stop_at_crossing) {
    ...
    // If all sensors have same value and value is lower than 0.2, it means we
    // are in all black scenario
  ...
  case mot_followline: {
    if (p->stop_criteria == stop_at_crossing && sensorIdxToFollow == -1) {
      p->motorspeed_r = 0;
      p->motorspeed_l = 0;
      p->finished = 1;
      break;
    ...
    if (p->stop_criteria == stop_at_left_line &&
        normalised_sensor_data[7] < 0.2) {
      p->motorspeed_r = 0;
      p->motorspeed_l = 0;
      p->finished = 1;
      break;
    ...
    if (p->stop_criteria == stop_at_right_line &&
        normalised_sensor_data[1] < 0.2) {
      p->motorspeed_r = 0;
      p->motorspeed_l = 0;
      p->finished = 1;
      break;
  ...
  case mot_followwall: {
    ...
    if (normalised_irsensor[sensor_index] > 0.3) {
      printf("Stopping following wall %.3f\n",
             normalised_irsensor[sensor_index]);
      p->motorspeed_r = 0;
      p->motorspeed_l = 0;
      p->finished = 1;
      break;
